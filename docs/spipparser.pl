...
printf("The current date is $date\n");
...
foreach my $lang (@ALL_LINGUAS) {
  ...
  my $name = "planegame";
  my $file = "$boards_dir/$name.xml";
  my $xslfile = "spip_oneboard.xsl";

  $article_id++;

  print "----------------------------------------\n";
  print "Applying the stylesheet for lang $lang and file $file (name=$name)\n";
  print "xsltproc  --stringparam language $lang --stringparam name ${name} $xslfile $file\n";

  my $output = `xsltproc --stringparam language $lang --stringparam name ${name} --stringparam date "${date}"  --stringparam article_id ${article_id} --stringparam rubrique_id $rubriques{$lang} --stringparam section_id $sections{$lang} $xslfile $file`;
