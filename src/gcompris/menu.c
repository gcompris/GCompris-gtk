			 ...
			 GcomprisBoard *gcomprisBoard, gboolean db)
{
  ...
  for (xmlnode = xmlnode->xmlChildrenNode; xmlnode != NULL; xmlnode = xmlnode->next) {
	...
	gcomprisBoard->title = reactivate_newline(_(title));
      }
	...
	gcomprisBoard->description = reactivate_newline(_(description));
      }
        ...
        const gchar *one_dirent, gboolean db)
{
	...
	if (board_read) {
	  ...
	  if (board_read) {
	    gc_status_set_msg( D_(GETTEXT_GUI,"Loading activity from file:\n%s"),
			       board_read->title );
	  }
	}
    else
