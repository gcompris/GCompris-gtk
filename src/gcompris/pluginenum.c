...
void init_plugins(void)
{
   ...
   * first                                                - Zinx
   */
  dir = g_strconcat(g_get_home_dir(), "/.gcompris/Plugins/", NULL);
  scan_plugins(dir);
  g_free(dir);
#endif

  /* Check is plugins are somewhere around us in the development branch */
  scan_plugins("../boards/.libs/");
  scan_plugins("./boards/.libs/");
  scan_plugins("./src/boards/.libs/");

  /* Now load the regularly installed plugins */
  dir = g_strconcat(PLUGIN_DIR, "/", NULL);
  scan_plugins(dir);
  g_free(dir);
